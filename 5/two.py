...
class Line:
    ...
    def diagonal_length(self):
        return max(abs(self.c1.x - self.c2.x), abs(self.c1.y - self.c2.y)) + 1

    def bottom_top_coordinates(self):
        if self.max_y_coordinate() == self.c1.y:
...
class Field:
    ...
    def draw_straight_line(self, line: Line):
        if line.horizontal():
    ...
    def draw_diagonal_line(self, line: Line):
        bottom, top = line.bottom_top_coordinates()
        if bottom.x < top.x:
            coords = list(
                zip([x for x in range(bottom.x, top.x + 1)], reversed([x for x in range(top.y, bottom.y + 1)])))
        else:
            coords = list(zip([x for x in range(top.x, bottom.x + 1)], [x for x in range(top.y, bottom.y + 1)]))
        for x in coords:
